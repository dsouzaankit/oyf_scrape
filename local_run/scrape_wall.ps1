...
$maximizeConsole = Join-Path $PSScriptRoot 'local_setup\maximize_console.ps1'
. $maximizeConsole
if (Restart-InConHostIfNeeded -ScriptPath $PSCommandPath) { exit 0 }
Write-Host 'Running in maximized console window.'
...
$LogPath = Join-Path $LogsFolder ("scrape_wall_{0:yyyyMMddTHHmmss}.log" -f (Get-Date))
...
try {
    ...
    $exitCode = Invoke-NodeScrape -NodeArgs @((Join-Path $RepoRoot 'node_script\web_scrape.js'), 'wall') -LogPath $LogPath
