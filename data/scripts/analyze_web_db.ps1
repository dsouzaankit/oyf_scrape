...
$LogPath = Join-Path $LogsFolder ("analyze_web_db_{0:yyyyMMddTHHmmss}.log" -f (Get-Date))
...
# Extra args are forwarded to the script (e.g. --deep). Read-only open, so safe while scraping.
$prevEap = $ErrorActionPreference
$ErrorActionPreference = 'Continue'
& node (Join-Path $PSScriptRoot 'analyze_web_db.js') $DbPath @args 2>&1 | ForEach-Object {
    $line = $_.ToString()
    Write-Host $line
...
if ($exitCode -ne 0) {
    Write-Host "analyze_web_db failed (exit $exitCode)."
}
