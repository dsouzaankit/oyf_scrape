# Run media_origin_date_tracker_multi_author_purchases.sql for standard origin-day windows.
#
# Usage:
#   .\run_media_origin_tracker_by_days_purchases.ps1
#   .\run_media_origin_tracker_by_days_purchases.ps1 -Days 30,90
#   .\run_media_origin_tracker_by_days_purchases.ps1 -AuthorId 180951488
...
if (-not $SqlPath) {
    $SqlPath = Join-Path $HomeDirectory 'sql_script\media_origin_date_tracker_multi_author_purchases.sql'
}
...
try {
    foreach ($dayWindow in $Days) {
        ...
        Write-Host "unlocked media_id with approx_origin_date within last $dayWindow days"
        Write-Host ('=' * 72)
