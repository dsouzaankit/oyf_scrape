...
-- Optional origin year filter: empty list = all years; add years (approx_origin_date) to restrict.
...
, origin_year_filter as (
	select 2026 as min_origin_year
	-- select null::integer as min_origin_year   -- all years
)
, t12 as (
select id chat_id
...
and (
	(select min_origin_year from origin_year_filter) is null
	or year(t2g.posted_date) >= (select min_origin_year from origin_year_filter)
)
